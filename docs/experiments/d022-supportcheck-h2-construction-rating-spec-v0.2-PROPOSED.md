# D-022 supportCheck — H2 Construction and Rating Operational Specification v0.2
...
- Predecessor: `d022-supportcheck-h2-construction-rating-spec-v0.1.md`, exact head `cb08182e9b8eb0e574f90a7b415e62d59e2222f0` (PR #133).
- Accepted upstream preregistration: `d022-supportcheck-h2-preregistration-v0.1.md`, exact head `f0ca5e6bd2328ab61b4d34368cac2b6d3dd4c391` (PR #132). Independent methodology disposition: ACCEPTED.
- v0.1 independent methodology disposition: CHANGES REQUIRED.
- v0.2 resolves the independent review's two blocking findings (B1: H1 claim-level exposure boundary; B2: checker-isolation timing) and incorporates compatible non-blocking hardening. No accepted PR #132 design rule, selection rule, pool composition, acceptance criterion, consensus rule, statistical threshold, scenario architecture, template cap, or stop rule is altered.
- No H2 authorship is authorized by this revision.
...
Rules:
- the attester must be an operator with direct access to the relevant session/account execution record; an H1/H2 rater, H2 author, inventory/taxonomy constructor or reviewer, auditor, checker implementer, validator reviewer, rating-instrument author, rubric-delta reviewer, synthetic-qualification-set author, or any other holder of a substantive or governance role in the role-separation matrix may not attest;
...
The inventory constructor and inventory reviewer must not have access to H1 claim-level ratings, consensus rows, disagreement rows, or other claim-level rating outcomes. Their work must use only the frozen H1 pool text and construction metadata. Joining H1 structural-inventory membership or claim IDs with H1 rating, consensus, disagreement, or other claim-level outcome data is prohibited for any role at any time.
...
- rubric-delta reviewer (responsible for determining whether H1→H2 rubric changes are format-only);
...
### H1 claim-level exposure prohibition

Prior access to H1 claim-level ratings, consensus rows, disagreement rows, or equivalent claim-level rating outcomes is disqualifying for the following roles:
- H2 taxonomy author;
- H2 taxonomy reviewer;
- rating-instrument/prompt author;
- rubric-delta reviewer;
- synthetic qualification-set author;
- H2 author;
- H2 construction/template auditor;
- H2 primary rater R1;
...
Each of these roles must provide an explicit non-exposure attestation before assuming the role. The attestation must state that the individual or model instance has not accessed, inspected, or received H1 claim-level ratings, consensus rows, disagreement rows, or equivalent claim-level rating outcomes in any form. These attestations are recorded in the role roster and verified during freeze-manifest review.

Joining H1 structural-inventory membership or claim IDs with H1 rating, consensus, disagreement, or other claim-level outcome data is prohibited for any role at any time and constitutes an exposure violation.

### Checker isolation

Parallel checker development is permitted only if isolation is mechanically or attestably established **before the checker implementer can receive or access any H2 prerequisite, construction, rating, taxonomy, qualification, pool, QA, or holdout artifact that could disclose what H2 will test**. Choose exactly one mechanism in the freeze manifest:

1. **checker-first freeze:** checker code/configuration/prompt and dependency hashes are committed and independently reviewed **before any H2 prerequisite artifact — including this operational specification, the H2 reasoning-template taxonomy, the rating instrument/prompt, the synthetic qualification set, the H2 pool, and all construction/QA metadata — is committed to any repository, workspace, or artifact store accessible to the checker implementer**; the checker implementer also provides an independently reviewed attestation of non-access to all prohibited H1 artifacts (except aggregate methodology statistics explicitly quoted in the closed H1 intake report) and non-access to all H2 artifacts enumerated above; or

2. **workspace exclusion:** an independently reviewed attestation identifies a workspace/account that cannot access the LibEdge repository, any H2 prerequisite/construction/rating/taxonomy/qualification/pool/QA artifacts, and any H1 artifacts beyond the permitted aggregate intake statistics; the attestation and checker freeze hash are recorded before access is later broadened.

The checker freeze identity/hash must be bound into the canonical freeze manifest. The freeze-manifest reviewer must verify that the chosen mechanism prevents any exposure to H2 taxonomy, template definitions, rating instrument, qualification material, pool content, QA metadata, or holdout artifacts before the checker freeze point.

The checker implementer must explicitly attest non-access to:
- all H1 artifacts except aggregate methodology statistics explicitly quoted in the closed H1 intake report;
- all H2 prerequisite artifacts (this specification, taxonomy, rating instrument, qualification set);
- all H2 construction, pool, QA, rating, consensus, and holdout artifacts.

This attestation is recorded in the role roster. Any violation stops H2 and requires a successor version.
...
The H2 taxonomy author and H2 taxonomy reviewer must each hold explicit non-exposure attestations for H1 claim-level ratings, consensus rows, and disagreement rows as specified in the H1 claim-level exposure prohibition section.
...
The canonical pool hash used to construct the blind bundle must equal the canonical-pool component hash of the final passing audit snapshot. All required construction validators — including the blind-leak validator — must pass before the relevant audit snapshot can become the final passing snapshot. If the blind-leak validator fails after pool freeze, the pool must not be mutated; instead H2 construction fails and requires a successor version.
...
Selection walk for every audit class: traverse the ranked list from first to last; select a claim only if its scenario has not yet been selected for that class; stop at the class quota. Rubric quotas are 24/24/12 as preregistered; every template-assignment class selects exactly 12 claims from exactly 12 distinct scenarios. Failure to obtain the required distinct scenarios is construction failure.
...
The revision log must list each defect, the frozen rule violated, whether the affected set was mechanically derived or auditor-approved, the complete affected-record set, auditor approval where required, the before/after hashes, and an explanation of why the mechanically derived or auditor-approved affected set fully covers the violated frozen rule. A deterministic diff-scope validator fails if a revision changes any record outside those logged affected sets or changes any frozen rule/taxonomy/audit artifact.
...
- role/exposure attestations, including H1 claim-level non-exposure attestations for the rating-instrument/prompt author, rubric-delta reviewer, and synthetic qualification-set author;
- exact evaluator prompt + rubric bytes and SHA-256;
- exact output JSON Schema and SHA-256;
- rating validator + tests and SHA-256;
- interface/provider endpoint and model configuration: temperature, max output tokens, thinking/reasoning mode, tools disabled, system/developer-message policy, streaming mode (non-streaming must be used where the interface supports it), and any other generation parameter exposed by the interface;
...
**Rubric semantics are frozen to H1.** H2 evaluator instructions may change only for format/transport/batching mechanics required by this spec. Every textual delta from the H1 semantic rubric must be enumerated with a finding by the designated rubric-delta reviewer that it is format-only and does not alter SUPPORTED/PARTIALLY_SUPPORTED/UNSUPPORTED semantics. The rubric-delta reviewer must hold an H1 claim-level non-exposure attestation and may not hold any other substantive or governance role.
...
A blind-leak validator must fail if any serialized blind item contains a source scenario/claim/evidence identifier or stratum marker, including substrings matching `H2-S[0-9]{3}`, `D022-H2-S[0-9]{3}-C[0-9]{2}`, source `:e[0-9]+` identifiers, or frozen stratum IDs. It also verifies every claim citation resolves only to local `E##` IDs. The blind-leak validator must pass before an audit snapshot containing the canonical pool can become the final passing audit snapshot. If the blind-leak validator fails after pool freeze, the pool must not be mutated; H2 construction fails and requires a successor version.
...
Non-streaming generation must be used where the interface supports it. Provider request identifiers, attempt identifiers, and any available generation-start/completion evidence must be preserved for each attempt.
...
All retries must be recorded. The final report must disclose retry incidence: total first-attempt failures, total retries, and per-batch/per-rater outcome (first-attempt valid / retry valid / retry invalid).
...
The checker implementer must not receive or access any H2 prerequisite artifact — including this operational specification, the H2 reasoning-template taxonomy, the rating instrument/prompt, the synthetic qualification set, the H2 pool, construction/QA metadata, rating outputs, consensus rows, or holdout subset — before the checker code/configuration/prompt is frozen and independently reviewed. The checker-isolation mechanism in the freeze manifest must be verified to prevent such access.

## Limitations disclosure

The final report must disclose at minimum the following limitations:

1. **Checker/rater lineage overlap:** any overlap between the candidate checker's runtime model lineage and a primary rater's model family.
2. **Substantive-role/rater family overlap:** any overlap where a holder of a substantive role (H2 author, construction auditor, taxonomy author, taxonomy reviewer, rating-instrument/prompt author, rubric-delta reviewer, or synthetic qualification-set author) shares a model family with a primary rater.
3. **Structural-equivalence exclusion:** the H1 structural-equivalence exclusion can narrow the effective estimand to non-H1-template transformations; the holdout result applies to the H2 template vocabulary, which by design excludes H1-equivalent transformations.
4. **Lexicographic selection channel:** lexicographic primary-subset selection creates an author-ordering channel; authors who know the selection rule can in principle bias which eligible claims enter the primary subset through claim-ID assignment, though claim IDs are assigned by scenario architecture and the blind-bundle permutation mitigates rater-order effects.
5. **Confounding of intended label, scenario architecture, and evidence packs:** intended label and scenario architecture are confounded; shared EvidencePacks within a scenario can create correlated rater cues across the six claims drawn from that scenario, so rater agreement within a scenario may not be independent.
6. **Clustering:** the accepted preregistration's clustering limitation (claims within a scenario are not independent Bernoulli trials) must be retained.
7. **Audit screen scope:** the audit is a gross-failure screen, not statistical assurance of a low unsampled defect rate.

These limitations must be presented as disclosed structural properties of the accepted design, not as defects requiring design changes.
...
- H1 claim-level exposure is discovered for any role subject to the H1 claim-level exposure prohibition;
...
- checker implementer is found to have accessed H2 prerequisite, taxonomy, rating instrument, qualification, pool, QA, or holdout artifacts before checker freeze;
- the blind-leak validator fails after pool freeze.
...
7. the canonical freeze manifest, including role roster, all exposure/non-access attestations, checker-isolation evidence with checker freeze hash, repository non-rewrite evidence and all prerequisite hashes/merge commits, is independently accepted and merged.
