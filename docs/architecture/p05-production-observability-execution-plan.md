...
Status: `PROPOSED / REVIEW REQUIRED / NOT EXECUTED`
...
A separately authorized, single synthetic invocation is used only to verify the effective log surface before starting the observation window.

The probe must be designed so that:

- it is read-only at the application level;
- it uses a unique non-sensitive sentinel in the URL query string solely to test redaction;
- it is authenticated only if authentication is required to exercise the same relevant request surface;
...
The reviewer must approve the exact endpoint and exact probe shape before execution.
...
4. no clear-text `Authorization` header value is present;
5. no clear-text `Cookie` header value is present;
...
No capacity-rate conclusion may be drawn from logs until this gate passes.
...
- exact authorized privacy-probe shape;
- proof that query-string sentinel is absent from logs;
- proof that `Authorization`/`Cookie` values are not exposed in clear text;
...
- clear-text `Authorization` or `Cookie` values appear in logs;
...
Acceptance of this execution plan would authorize **only** a later, explicitly approved production observability configuration execution under the exact constraints above.
...
3. whether the privacy probe needs a more specific endpoint/shape before deployment authorization;
4. whether the query-string and `Authorization`/`Cookie` verification is strong enough;
5. whether any other default Workers Logs field must be explicitly inspected;
6. whether rollback is narrow and safe;
7. whether observation-window duration is correctly left for a separate review;
8. whether this plan is ready to proceed to a separate explicit production execution authorization.
