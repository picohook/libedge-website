...
import { recordResearchMetric, recordSemanticTelemetry } from './telemetry.js';
...
function openAlexSuccessMeta(mode, telemetry, semanticError = null, cached = false) {
  if (mode === 'semantic') {
    return { status: 'ok', mode: 'semantic', telemetry, ...(cached ? { cached: true } : {}) };
  }
  if (semanticError) {
    return {
      status: 'ok',
...
function openAlexDualFailureMeta(semanticError, lexicalError) {
  return {
    ...
    stages: {
      semantic: openAlexProviderMeta(semanticError),
      lexical: openAlexProviderMeta(lexicalError)
    }
  };
}
...
async function recordSemanticFailure(env, error) {
  if (error?.code === 'OPENALEX_MALFORMED') return recordResearchMetric(env, 'semantic_malformed');
  if (error?.code === 'OPENALEX_RATE_LIMITED') return recordResearchMetric(env, 'semantic_429');
  if (error?.code === 'OPENALEX_SEMANTIC_PACING_UNAVAILABLE') return recordResearchMetric(env, 'semantic_pacing_unavailable');
  if (error?.name === 'AbortError' || error instanceof TypeError) return recordResearchMetric(env, 'semantic_timeout_network');
  if (Number(error?.status) >= 500) return recordResearchMetric(env, 'semantic_5xx');
}
    ...
    return {
      response: {
        ...
        meta: {
          ...
          cached: true,
          providers: {
            ...cached.meta?.providers,
            openalex: openAlexMeta,
            crossref: { status: 'ok', cached: true }
          }
        }
      }
    };
...
async function buildOpenAlexPayload(openAlex, env, mode, semanticError = null, perPage = 10) {
  ...
  return {
    ...
    meta: {
      ...
      providers: {
        openalex: openAlexSuccessMeta(mode, openAlex.telemetry, semanticError),
...
app.get('/api/research/search', async (c) => {
  ...
  try {
    await recordResearchMetric(c.env, 'semantic_attempts');
    const pacing = await acquireSemanticPacing(c.env);
    await recordResearchMetric(c.env, 'semantic_pacing_wait_ms_total', pacing.waitMs);
    ...
    await recordResearchMetric(c.env, 'semantic_provider_latency_ms_total', Date.now() - startedAt);
    await recordResearchMetric(c.env, 'semantic_successes');
    if (semantic.results.length === 0) await recordResearchMetric(c.env, 'semantic_valid_empty');
    await recordSemanticTelemetry(c.env, semantic.telemetry);
    ...
    semanticError = error;
    await recordSemanticFailure(c.env, error);
    if (!isSemanticAvailabilityFailure(error)) {
      return c.json({
        error: 'Anlamsal akademik arama şu anda işlenemiyor',
        code: 'RESEARCH_SEMANTIC_UNAVAILABLE',
        meta: {
          partial: false,
          cached: false,
          providers: { openalex: openAlexProviderMeta(error) }
        }
      }, 503);
    }
  }
  ...
  if (cachedLexical) {
    ...
    await recordResearchMetric(c.env, 'lexical_fallback_successes');
    return c.json({
      ...cachedLexical,
      meta: {
        ...cachedLexical.meta,
        ...
        providers: {
          ...cachedLexical.meta?.providers,
          openalex: openAlexSuccessMeta('lexical', cachedTelemetry, semanticError, true)
        }
      }
    });
  }
