...
app.post('/api/admin/announcements/ai/image', async (c) => {
  ...
  try {
    ...
    if (!cleanTitle) return c.json({ error: 'Ba�l�k zorunludur' }, 400);
    ...
    return c.json({ error: err.message || 'G�rsel olu�turulamad�' }, 500);
  }
});
...
app.post('/api/admin/announcements/ai/polish', async (c) => {
  ...
  try {
    ...
    if (!cleanAnnouncementText(payload.title) && !cleanAnnouncementText(payload.full_content) && !cleanAnnouncementText(payload.summary)) {
      return c.json({ error: 'D�zenlenecek metin bulunamad�' }, 400);
    }
    ...
    return c.json({ error: err.message || '��erik d�zenlenemedi' }, 500);
  }
});
...
app.post('/api/admin/announcements/ai/translate', async (c) => {
  ...
  try {
    const payload = await c.req.json();
    if (!cleanAnnouncementText(payload.title) && !cleanAnnouncementText(payload.full_content) && !cleanAnnouncementText(payload.summary)) {
      return c.json({ error: '�evrilecek metin bulunamad�' }, 400);
    }
    ...
    return c.json({ error: err.message || '�eviri yap�lamad�' }, 500);
  }
});
...
app.post('/api/admin/announcements', async (c) => {
  ...
  if (!title) return c.json({ error: 'Ba�l�k zorunludur' }, 400);
...
app.put('/api/admin/announcements/:id', async (c) => {
  ...
  try {
    ...
    if (result.meta?.changes === 0) return c.json({ error: 'Duyuru bulunamad�' }, 404);
    return c.json({ success: true });
  } catch (err) {
