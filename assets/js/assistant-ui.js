import('./assistant-ui-state.js').then(({ loadingStage, mapAssistantResult }) => {
    ...
    function setStatus({ title, message, tone = 'info', marker = '', state = '' }) {
        if (!status) return;
        const tones = { success:['#f0fdf4','#bbf7d0','#166534'], loading:['#eff6ff','#bfdbfe','#1d4ed8'], notice:['#f5f3ff','#ddd6fe','#6d28d9'], warning:['#fffbeb','#fde68a','#92400e'], error:['#fef2f2','#fecaca','#b91c1c'], info:['#eff6ff','#bfdbfe','#1d4ed8'] };
    ...
    function ensureSourceDetail(sourceCard, button) {
        ...
        detail.style.marginTop = '12px'; detail.style.padding = '12px'; detail.style.border = '1px solid #dbeafe'; detail.style.borderRadius = '10px'; detail.style.background = '#f8fbff';
    ...
    function relatedFindingsForSource(sourceId) { return [...document.querySelectorAll('.finding-item')].filter((finding) => finding.querySelector(`.citation-chip[data-source="${sourceId}"]`)); }
    ...
    function focusFindingEvidence(finding) {
        const sourceIds = [...finding.querySelectorAll('.citation-chip[data-source]')].map((chip) => chip.dataset.source).filter((sourceId, index, all) => sourceId && all.indexOf(sourceId) === index);
    ...
    function bindFindingEvidenceInteractions() {
        document.querySelectorAll('.finding-item').forEach((finding) => {
            ...
            finding.tabIndex = 0; finding.setAttribute('role', 'button'); finding.setAttribute('aria-pressed', 'false'); finding.setAttribute('aria-label', `Bulgu için ${sourceCount} ilişkili kanıt kaydını göster`);
    ...
    document.querySelectorAll('.assistant-answer-actions button:not(#viewSourcesBtn)').forEach((button) => { if (!button.disabled) button.addEventListener('click', () => showToast('Bu aksiyon sonraki UI iterasyonunda etkinleştirilecek.')); });
