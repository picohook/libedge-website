...
const ratingKeys=["item_id","label"], labels=new Set(["SUPPORTED","PARTIALLY_SUPPORTED","UNSUPPORTED"]);
const errors=[];
...
if(!Array.isArray(out.ratings)||out.ratings.length!==720) errors.push("ratings_count");
 ...
 const ids=bundle.claims.map(x=>x.item_id);
 const seen=new Set();
 for(let i=0;i<out.ratings.length;i++){
  ...
  if(r.item_id!==ids[i]) errors.push(`order_or_id:${i}`);
  if(seen.has(r.item_id)) errors.push(`duplicate:${r.item_id}`);
  seen.add(r.item_id);
  if(!labels.has(r.label)) errors.push(`label:${r.item_id}`);
 }
}
